...
const sendOtpMail = async (user, otp) => {
  ...
  await sendEmail(user.email, "Digiweb Star — OTP Verification", html);
};

const sendRegisterationMail = async (user) => {
  const html = registerTemplate(user.full_name);
  await sendEmail(user.email, "Welcome to Digiweb Star Solution!", html);
};
...
userRouter.post("/register", async (req, res) => {
  ...
  type: "achievement",
    ...
    return res
      ...
      .json({
        success: true,
        message: "Registered at Digiweb Star! Please check your email for OTP.",
      });
  } catch (err) {
    console.error(err);
